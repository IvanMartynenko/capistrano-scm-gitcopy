namespace :gitcopy do
  ...
  task :wrapper do
    run_locally do
      ...
      File.write("#{fetch(:tmp_dir)}/#{fetch(:application)}/git-ssh.sh",
  ...
  desc 'Create tarfile'
  task create_tarfile: %i[gitcopy:update gitcopy:set_current_revision] do
    local_path = fetch(:local_path)
  ...
  task create_release: :'gitcopy:create_tarfile' do
    ...
    run_locally do
      execute :rm, strategy.local_tarfile if File.exist? strategy.local_tarfile
    end
  end
